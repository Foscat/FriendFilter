...
module.exports = {
  ...
  update: function(req, res) {
    ...
    if(!req.params.id || req.body === {}){
      ...
      let score = req.body.surveyScore;
      if(Object.keys(req.body).includes("surveyScore")){
        req.body.personalityType = "";
        if(score > 0){
          if(score <= 20) req.body.personalityType = "Toadal friggn jeanyus"
          if(score > 20 && score <= 30) req.body.personalityType = "Wan fart smeller"
          if(score > 30 && score <= 40) req.body.personalityType = "Fence rider"
          if(score > 40 && score <= 50) req.body.personalityType = "Party pooper"
          if(score > 50 && score <= 60) req.body.personalityType = "Jhonny da dull boy"
    ...
    db.User.find({username: req.body.username})
      .then((data,err) => {
        ...
        if(hashedInput === data[0].password){
          ...
          return res.send({message:"Token recived", info:token, user: data})
        }
        else return res.send({message:"Something went wrong.", info:{dataObj:data, errObj:err}})
      })
      .catch(err => res.send({message:"A user was not found by that name.", info: err}));
  },
  findMaches: function(req,res){
    console.log("Find matches request");
    db.User.find({personalityType: req.body.personalityType}).then((data,err) => {
      if(err) res.send({message:"There was an error", info:err});
      if(Object.keys(data).length <= 0)res.send({message:"There is no data in response.", info:data});
      console.log("It worked!", data);
      return res.send({message:"Here are you matches", info:data});
    }).catch(err => res.send({message:"A error was caught", info: err}));
  }
};